...
use essentia_core::{algorithm::ParameterInfo, Introspection};
use proc_macro2::TokenStream;
use quote::{format_ident, quote};

use crate::algorithm_generation::common::{data_type_to_phantom, sanitize_identifier_string, string_to_doc_comment};
...
pub fn generate_parameter_functions(
    algorithm_introspection: &Introspection,
) -> Vec<TokenStream> {
    algorithm_introspection
        ...
        .map(|parameter| {
            ...
            let variant = data_type_to_phantom(&parameter.parameter_type().into());
