use crate::variant_data::ConversionError;
use crate::variant_data::from_other::IntoVariantData;
use crate::variant_data::into_other::{GetVariantData, TryGetVariantData};
use crate::{ffi, pool::Pool};
...
fn test_bool() {
    let value = true;
    let variant = value.into_variant_data();
    assert_eq!(variant.get(), value);
}
...
fn test_stereo_sample() {
    ...
    let variant = value.clone().into_variant_data();
    let got = variant.get();
...
fn test_vector_bool() {
    ...
    let variant = value.as_slice().into_variant_data();
    assert_eq!(variant.get(), value);
}
...
fn test_vector_string() {
    ...
    assert_eq!(variant.get(), expected);
}
...
fn test_vector_vector_float_to_matrix() {
    let value = vec![vec![1.1, 2.2], vec![3.3, 4.4f32]];
    let variant = value.as_slice().into_variant_data();
    let matrix: Result<Array2<f32>, _> = variant.try_get();
...
fn test_vector_vector_string() {
    ...
    let variant = slices.as_slice().into_variant_data();
...
fn test_pool() {
    ...
    let variant = pool.into_variant_data();
    let retrieved_pool: Pool = variant.get();
...
fn test_vector_vector_float_to_matrix_empty() {
    ...
    let variant = value.as_slice().into_variant_data();
    let matrix: Result<Array2<f32>, ConversionError> = variant.try_get();
